...
test("passing props and refs", async () => {
  const MyComponentWithRef = React.forwardRef((props, ref) => (
    <div ref={ref}>MyComponent {JSON.stringify(props)}</div>
  ));

  const MyLoadingComponentWithRef = React.forwardRef((props, ref) => (
    <div ref={ref}>MyLoadingComponent {JSON.stringify(props)}</div>
  ));

  const refCallback = jest.fn();

  let LoadableMyComponent = Loadable({
    loader: createLoader(400, () => MyComponentWithRef),
    loading: MyLoadingComponentWithRef
  });
  let component = renderer.create(
    <LoadableMyComponent prop="baz" ref={refCallback} />
  );
  expect(component.toJSON()).toMatchSnapshot(); // initial
  await waitFor(200);
  expect(component.toJSON()).toMatchSnapshot(); // loading
